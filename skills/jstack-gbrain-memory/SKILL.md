---
name: jstack-gbrain-memory
description: "Use when turning J-Stack story artifacts into durable GBrain knowledge: story hubs, source cards, claim pages, evidence pages, entity graph links, timelines, corrections, and reusable background."
version: 1.1.0
author: J-Stack project for Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [jstack, gbrain, journalism, gbrain]
    related_skills: [jstack-command-center]
---

# J-Stack GBrain Memory

## Overview

Make GBrain the newsroom memory and knowledge graph for investigations.
...
- A J-Stack workflow needs persistence, retrieval, linking, or later reuse.
- A dossier or article package should become searchable and graph-linked.

## Do Not Use For

- Do not store secrets, raw credentials, unnecessary personal data, or avoidable sensitive dumps.
- Do not make GBrain a substitute for source archives.

## Role in the J-Stack Newsroom

- **Skill family:** `gbrain`
- **Primary responsibility:** Make GBrain the newsroom memory and knowledge graph for investigations.
...
- Link plan
- Tags/timeline plan
- Fallback markdown package

## Workflow

1. Classify artifacts into story, source, claim, evidence, entity, timeline, draft, risk, and postpub pages.
2. Choose safe slugs and tags.
3. Write concise non-secret pages and links.
4. Verify direct readback plus search/query.
5. If GBrain is unavailable, save markdown pages and manifest for later import.

## GBrain Integration

### Reads from GBrain

- `all related story/project pages`
- `existing entity/source/claim pages`
- `backlinks and timelines`

### Writes or Proposes in GBrain

- story hub and artifact pages
- typed links: supports, contradicts, mentions, part_of, challenges, has_source, has_claim
- timeline entries for durable events
...
1. **Dumping too much raw evidence into GBrain.**
2. **Creating orphan pages.**
3. **Not verifying searchability after import.**
...
```text
Use jstack-gbrain-memory on <story/topic>.
