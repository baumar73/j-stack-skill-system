---
name: jstack-source-map
description: "Use when building a complete source universe for a story: primary sources, secondary sources, adversarial sources, expert sources, missing sources, and archive status."
version: 1.1.0
...
Create a defensible source map rather than a pile of links.
...
- A factual story needs source planning.
- The user has links but no reliability hierarchy.
- A controversial claim requires primary and adversarial evidence.

## Do Not Use For

- Do not treat unvisited search results as sources.
- Do not let a source prove more than it actually proves.

## Role in the J-Stack Newsroom

- **Skill family:** `research`
- **Primary responsibility:** Create a defensible source map rather than a pile of links.
...
1. Define source classes needed for each subclaim.
2. Rank sources by authority, independence, proximity, bias, freshness, and what they prove.
3. Mark each source’s limits: what it cannot prove.
4. Archive or record retrieval metadata where lawful and practical.
5. Link each source to claim IDs.
...
- sources/<story>/<source-id> cards
- links source -> supports/contradicts claim
- tags for source class and reliability
...
1. **Overweighting journalism that merely quotes another story.**
2. **Ignoring language/translation risk.**
3. **Failing to capture a missing primary source.**
...
```text
Use jstack-source-map on <story/topic>.
